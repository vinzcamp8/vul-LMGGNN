...
def run_gat(args, train_loader, val_loader, test_loader):
    ...
    path_output_model = f"{PATHS.model}gat_{learning_rate}_{batch_size}_{epochs}_{weight_decay}/"
    ...
    for epoch in range(1, epochs + 1):
        ...
        if f1 > best_f1 or (f1 == best_f1 and recall > best_recall):
            ...
            checkpoint_path = str(path_output_model+"gat_checkpoint.pth")
    ...
    # Test the model
    model_test = GAT(input_dim, hidden_dim, output_dim, heads=heads).to(DEVICE)
    path_checkpoint = str(path_output_model+"gat_checkpoint.pth")
